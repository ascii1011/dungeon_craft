...
func before_each() -> void:
	...
	# Inject mock item data so DataLoader doesn't need disk access
	DataLoader.set_mock_item("sword_iron", {
		"id": "sword_iron",
		"name": "Iron Sword",
		"value": 50,
		"type": "weapon",
...
func test_buy_reduces_player_gold() -> void:
	economy_system.buy_item(inventory, "sword_iron", shop_data)
...
func test_buy_fails_insufficient_gold() -> void:
	...
	var result: bool = economy_system.buy_item(inventory, "sword_iron", shop_data)
...
func test_sell_increases_gold() -> void:
	...
	economy_system.sell_item(inventory, "sword_iron", shop_data)
